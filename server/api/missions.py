...
async def generate_manual_mission(body: ManualMissionRequest) -> GridResponse:
    ...
    try:
        if body.upload:
            result = mission_service.load_generated(mission, enrich=False)
            uploaded = result["uploaded_to_drone"]
            verified = result["verified"]
            verify_msg = result["verification_message"]
        else:
            mission_service.store_mission(mission, enrich=False)
    except MissionUploadError as exc:
        logger.error("Manual mission upload failed: %s", exc)
        raise HTTPException(status_code=502, detail=f"MAVLink upload error: {exc}")

    mission = mission_service.current_mission or mission

    if uploaded and verified:
        msg = "Manual mission generated, uploaded, and verified on vehicle."
    elif uploaded:
        msg = f"Manual mission uploaded but verification failed: {verify_msg}"
    else:
        msg = "Manual mission generated. Connect to drone to upload."
